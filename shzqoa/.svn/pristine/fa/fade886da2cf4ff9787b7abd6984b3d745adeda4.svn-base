...
<c:if test="${fn:length(workTasks) > 0}">
	<table style="word-break:break-all;"  cellpadding="0" cellspacing="0" border="0" width="100%" class="whh_tab1">
		<thead>
			<tr>
			  	<th width="20%">任务</th><th width="80%">进度</th>
			</tr>
		</thead>
		<tbody>
...
<div class="zyh_rizhi_tan">
              
              	<div class="zyh_riz_tan">
               	  <div class="zyh_tan_kun">
              		<h1>${daily.publishcontent }</h1>
                    <p>${daily.logcontent}</p>
                    <p><b>问题建议：</b>${daily.propose }</p>
                    <p>
                    	<b>录入简历信息：</b>
                    	<c:forEach items= "${rescountlist}" var="item" >
                    		<c:forEach items="${resumeSourceList}" var="resitem">
                   					<c:if test="${resitem.resumesourceid==item.resumesourceid}">
                   						${resitem.resumesourcename}:${item.count}份
                   					</c:if>
                   				</c:forEach>
                    	</c:forEach>
                    </p>
