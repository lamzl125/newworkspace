...
function toprevpageInfo(page){
	...
	var url="/CustomerInfoKq/customerInfoKqList.do?page="+(page-1)+"&beginTime="+beginTime+"&endTime="+endTime+"&customerCode="+customerCode+"&usercode="+usercode;
...
function togetListBypage(page){
	...
	var customerCode=$("#customerCode").find("option:selected").val();
	var usercode=$("#userid").find("option:selected").val();
  	var url = "/CustomerInfoKq/customerInfoKqList.do?page="+page+"&beginTime="+beginTime+"&endTime="+endTime+"&customerCode="+customerCode+"&usercode="+usercode;
  	window.location.href=url;
  }
...
function topageInfo(){
	...
	var usercode=$("#userid").find("option:selected").val();
  	...
  	var url = "/CustomerInfoKq/customerInfoKqList.do?page="+topage+"&beginTime="+beginTime+"&endTime="+endTime+"&customerCode="+customerCode+"&usercode="+usercode;
  	window.location.href=url;
  } 
...
function searchByParam(){
	...
	var customerCode=$("#customerCode").find("option:selected").val();
	var usercode=$("#userid").find("option:selected").val();
	var url="/CustomerInfoKq/customerInfoKqList.do?beginTime="+beginTime+"&endTime="+endTime+"&customerCode="+customerCode+"&usercode="+usercode;
	window.location.href=url;
}
...
<div id="content">
	<div id="content-header">
	    <div id="breadcrumb"> 
	    	...
	    	<a href="" title="录入考勤" class="tip-bottom"><i class="icon-home"></i>录入考勤</a>
	     	<a href="" title="考勤列表" class="current">考勤列表</a>
	    </div>
	</div> 
        ...
        <div class="widget-box">
          ...
          <div class="widget-content clearfix">
            	<div class="zyh_sos">
					...
					<span>录入人员：</span> <select class="zyh_sel" id="userid"
            ...
            <table class="table table-bordered table-striped with-check">
              ...
              <thead>
                <tr>
                	<th>人员</th><th>考勤时间</th><th>是否转正</th><th>试用期实际时间</th>
					<th>闲置出勤天数</th><th>加班小时</th><th>请假天数</th><th>迟到早退次数</th>
					<th>实际出勤</th><th>法定出勤</th><th>税金</th><th>扣借款</th>
					<th>扣住宿费用</th><th>报销费用</th><th>保险</th><th>补助</th>
					<th>状态</th><th>考勤来源</th><th>录入人员</th><th>录入时间</th>
                </tr>
              </thead>
              <tbody>
           			<c:if test="${fn:length(list) == 0}">
						...
						<c:forEach items="${list }" var="info">
                   		...
                   		${cs.customername }
								...
								<td><fmt:formatDate pattern="yyyy-MM-dd" value="${info.kqtime }" /></td>
								<td item="${info.ifZZ }">${info.ifZZ eq 1?'已转正':info.ifZZ eq 2?'未转正':info.ifZZ eq 3?'月内转正':'' }</td>
								<td item="${info.syqTime }">${info.syqTime }</td>
								<td item="${info.xzTime }">${info.xzTime }</td>
								<td item="${info.jbHover }">${info.jbHover }</td>
								<td item="${info.qjTime }">${info.qjTime }</td>
								<td item="${info.cdTime }">${info.cdTime }</td>								
								<td item="${info.sjTime }">${info.sjTime }</td>
								<td item="${info.fdTime }">${info.fdTime }</td>
								<td item="${info.sj }">${info.sj }</td>
								<td item="${info.kjk }">${info.kjk }</td>
								<td item="${info.kzs }">${info.kzs }</td>
								<td item="${info.bxiao }">${info.bxiao }</td>
								<td item="${info.bxian }">${info.bxian }</td>
								<td item="${info.bz }">${info.bz }</td>								
								<td item="${info.status }"><c:if test="${info.status==0 }">错误</c:if>
									<c:if test="${info.status==1 }">正确</c:if></td>
								<td item="${info.kqSource }">${info.kqSource eq 1?'项上人员自己' :info.kqSource eq 2?'公司客户':''}</td>
								...
								<td><fmt:formatDate pattern="yyyy-MM-dd" value="${info.addtime }" /></td>
							</tr>
						</c:forEach>
              </tbody>
            </table>
