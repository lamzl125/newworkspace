...
<script type="text/javascript">
 	function togetListBypage(page){
		...
		var monthstr = $('.search').find('select[name="monthstr"]').val();
	 	var url = "/servicepaysummary/expectedreturn.do?page="+page+"&corpcode="+encodeURI(encodeURI($.trim(corpcode)))
	...
	function tonextpageInfo(page){
		...
		var yearstr = $('.search').find('input[name="yearstr"]').val();
		var monthstr = $('.search').find('select[name="monthstr"]').val();
	 	var url = "/servicepaysummary/expectedreturn.do?page="+(page+1)+"&corpcode="+encodeURI(encodeURI($.trim(corpcode)))
		+"&yearstr="+encodeURI(encodeURI($.trim(yearstr)))
		+"&monthstr="+encodeURI(encodeURI($.trim(monthstr)));
	 	window.location.href=url;
	}
	...
	function topageInfo(){
		...
		var monthstr = $('.search').find('select[name="monthstr"]').val();
	 	...
	 	var url = "/servicepaysummary/expectedreturn.do?page="+topage+"&corpcode="+encodeURI(encodeURI($.trim(corpcode)))
			+"&yearstr="+encodeURI(encodeURI($.trim(yearstr)))
			+"&monthstr="+encodeURI(encodeURI($.trim(monthstr)));
		window.location.href=url;
	}
	$(function(){
		$('.searchBth').off('click').on('click',function(){
			...
			var url = "/servicepaysummary/expectedreturn.do?corpcode="+encodeURI(encodeURI($.trim(corpcode)))
			+"&yearstr="+encodeURI(encodeURI($.trim(yearstr)))
			+"&monthstr="+encodeURI(encodeURI($.trim(monthstr)));
			window.location.href=url;
		});
        ...
        <div class="widget-box">
          ...
          <div class="widget-content clearfix">
            ...
            <table class="table table-bordered table-striped with-check">
              ...
              <thead>
                <tr>
                	<th>客户公司</th><th>摘要</th><th>金额</th><th>预计回款日期</th>
					<th>备注</th><th>沟通日期</th><th>沟通情况</th>
                </tr>
              </thead>
              <tbody>
           			<c:forEach items="${list}" var="info">
	    				<tr>
							...
							<td>${info.Amount}</td>
							<td><fmt:formatDate value="${info.PlanrRceivedPaymentsDate}" pattern="yyyy-MM-dd" /></td>
							<td>${info.PaymentContent }</td>
							<td><fmt:formatDate value="${info.FollowDate}" pattern="yyyy-MM-dd" /></td>
