 ...
 <head> 
  ...
  function togetListBypage(page){
	  ...
	  var enddate = $("#enddate").val();	
  	  var url = "/resume/resumestatisticslist.do?page="+page;
  ...
  function tonextpageInfo(page){
	  var startdate = $("#startdate").val();
	  var enddate = $("#enddate").val();	
  	  var url = "/resume/resumestatisticslist.do?page="+(page+1);
  	  if(startdate!=null && startdate!=""){
		  url += "&startdate="+encodeURI(encodeURI($.trim(startdate)));
	  }
	  if(enddate!=null && enddate!=""){
  ...
  function topageInfo(){
	  ...
	  var enddate = $("#enddate").val();	
  	  ...
  	  var url = "/resume/resumestatisticslist.do?page="+topage;
  	  if(startdate!=null && startdate!=""){
		  url += "&startdate="+encodeURI(encodeURI($.trim(startdate)));
	  }
	  if(enddate!=null && enddate!=""){
  ...
  function searchByCon(){
	  var startdate = $("#startdate").val();
	  var enddate = $("#enddate").val();	  
	  var url = "/resume/resumestatisticslist.do?page=1";
	  if(startdate!=null && startdate!=""){
		  url += "&startdate="+encodeURI(encodeURI($.trim(startdate)));
	  }
	  if(enddate!=null && enddate!=""){
  ...
  function customerstatuslist(userCode,optype){
	  var startdate = $("#startdate").val();
	  var enddate = $("#enddate").val();
	  if(optype==1){
		  var url = "/resume/customersourcelist.do?userCode="+userCode+"&optype="+optype;
		  if(startdate!=null && startdate!=""){
		  ...
		  var url = "/resume/customerlist.do?userCode="+userCode+"&optype="+optype;
		  if(startdate!=null && startdate!=""){
			  url += "&startdate="+startdate;
		  }
		  if(enddate!=null && enddate!=""){
        ...
        <div class="widget-box">
          ...
          <div class="widget-content clearfix">
            ...
            <table class="table table-bordered table-striped with-check">
              <thead>
                <tr>
                  <th>#</th><th>添加人</th><th>录入份数</th><th>可面人数</th><th>面试人数</th><th>面试通过人数</th><th>入项人数</th>         
                </tr>
              </thead>
              <tbody>
				...
				<c:forEach var="item" items="${stlist}" varStatus="status">
	                  <tr>
	                  	   ...
	                  	   <td>${item.realName}</td>
	                  	   <td><a class="zyh_font_color entrylist" href="javascript:void(0)" onclick="customerstatuslist('${item.userCode}',1)">${item.addcount}</a></td>
	                  	   <td><a class="zyh_font_color entrylist" href="javascript:void(0)" onclick="customerstatuslist('${item.userCode}',2)">${item.yjinterviewcount}</a></td>
	                  	   <td><a class="zyh_font_color entrylist" href="javascript:void(0)" onclick="customerstatuslist('${item.userCode}',3)">${item.interviewcount}</a></td>
	                  	   <td><a class="zyh_font_color entrylist" href="javascript:void(0)" onclick="customerstatuslist('${item.userCode}',4)">${item.successcount}</a></td>
	                  	   <td><a class="zyh_font_color entrylist" href="javascript:void(0)" onclick="customerstatuslist('${item.userCode}',5)">${item.joinprojectcount}</a></td>
	                 </tr>
                 </c:forEach>
              </tbody>
            </table>
