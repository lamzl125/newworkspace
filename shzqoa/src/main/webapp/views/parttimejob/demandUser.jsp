         ...
         <div class="whh_tab_bor">
			...
			<table width="45%" border="1" class="zyh_table" style="margin-left:0;margin-right:0px;" >
				<th colspan="3" class="zyh_th">抢/分配到的需求</th>
				   <c:forEach items="${list1}" var="demand" varStatus="status">
				   		<tr>
						  	<td class="zyh_wid">&nbsp;</td>
						    <td class="zyh_wid1" scope="row"><input type="checkbox" name="list1Check" <c:if test="${status.index==0}">checked</c:if> value="${demand.id}"></td>
						    <td class="zyh_wid2">
						    	${demand.projecttype}&nbsp;|
						    	${demand.corpcode}&nbsp;|&nbsp;
						    	<c:forEach items="${professList}" var="proinfo">
	                				<c:if test="${proinfo.id==demand.technologydirection}">
			                   			${proinfo.name }
			                   		</c:if>
		                   		</c:forEach>&nbsp;|&nbsp;
		                   		<c:forEach items="${areaList}" var="areainfo">
	                				<c:if test="${areainfo.areaid==demand.projectlocation}">
			                   			${areainfo.areaname }
			                   		</c:if>
		                   		</c:forEach>&nbsp;|&nbsp;
		                   		<c:forEach items="${gradList}" var="gradinfo">
	                				<c:if test="${gradinfo.id==demand.demandyears}">
			                   			${gradinfo.gradename }
			                   		</c:if>
		                   		</c:forEach>
						    </td>
			...
			<table width="30%" border="1" class="zyh_table" id="list2" style="float:right;">
				<th colspan="3" class="zyh_th">录入的简历</th>
					<c:forEach items="${list2}" var="resume">
				   		<tr>
						  	<td class="zyh_wid">&nbsp;</td>
						    <td class="zyh_wid1" scope="row">
						    	<input type="checkbox" name="list2Check" 
							    ...
							    value="${resume.customercode}">
						    </td>
						    <td class="zyh_wid2">
						    	<a href="/customerDatailInfo/toCustomerDatailInfo.do?customercode=${resume.customercode}">${resume.customername}&nbsp;|&nbsp;${resume.customertel}</a>
						    </td>
						  </tr>
				   </c:forEach>
			</table>
