...
@Controller
@RequestMapping(value="/partjob")
public class PartJobDemandController {
	@Resource
    ...
    public ModelAndView demandNoBindResumeList(
		...
		ModelAndView mv = new ModelAndView("views/parttimejob/addResume");	
...
//		map.put("pageSize", pageSize);		
    ...
    public ModelAndView demandBindResumeList(
    		...
    		@RequestParam(value = "pageSize", defaultValue = "10") int pageSize,
    		HttpServletRequest request) throws UnsupportedEncodingException{
		ModelAndView mv = new ModelAndView("views/parttimejob/demandbindresumelist");	
		customername = URLDecoder.decode(customername, "UTF-8");
		demandId = URLDecoder.decode(demandId, "UTF-8");
		mv.addObject("customername", customername);
		mv.addObject("demandId", demandId);
    ...
    public ModelAndView demandResumeList(
    		...
    		@RequestParam(value = "pageSize", defaultValue = "10") int pageSize,
    		HttpServletRequest request){
		ModelAndView mv = new ModelAndView("views/parttimejob/demandUser");
		User user = (User) request.getSession().getAttribute("user");
		
		//查询所有需求列表
		List<CustDemand> list=customerDemandService.selectOpenDemand(new HashMap<String,Object>());
		
		//当前用户上传的简历列表
		Map<String, Object> map = customerInfoService.selectCustInfoByOpertCode(page, 9999999, user.getUsercode());
		
		//获取当前用户提交简历绑定需求列表
		List<Map<String,Object>> alldemandResumelist = new ArrayList<Map<String,Object>>();//存放简历绑定需求
		
		//获取第一个需求绑定的简历
		if(list!=null && list.size()>0){
			CustDemand dean = list.get(0);
			alldemandResumelist = demandResumeService.selectDemandResumeByDemand(dean.getId());
		}
		List<CustomerInfo> culist = (List<CustomerInfo>) map.get("customerInfoList");		
//		if(culist!=null && culist.size()>0){
//			for(int i=0;i<culist.size();i++){
//				CustomerInfo custinfo = culist.get(i);
//				List<Map<String,Object>> demandResumelist = demandResumeService.selectDemandResumeByDemand(list);
//				alldemandResumelist.addAll(demandResumelist);
//			}
//		}
		...
		mv.addObject("list2", culist);
		mv.addObject("list3", alldemandResumelist);
		return mv;
	}
    ...
    public ModelAndView demandList(
		...
		ModelAndView mv = new ModelAndView("views/parttimejob/custDemandList");
		mv.addObject("currpage", page);
		...
		//统计需求对应的简历数量
		Map<String,Integer> demandrescount = new HashMap<String,Integer>();
		if(custList!=null && custList.size()>0){
			for(int i=0;i<custList.size();i++){
				Map<String,Object> finmap = new HashMap<String,Object>();
				CustDemand info = custList.get(i);
				finmap.put("demandId", info.getId());
				int count = demandResumeService.demandResumeOfDemandCount(finmap);
				demandrescount.put(info.getId(), count);
			}
		}
		mv.addObject("demandrescount",demandrescount);
		//统计筛选时长,统计通知面试结果时长,统计安排入项时长
		List<Map<String,Object>> durationlist = demandResumeFollowService.getDurationByCorp(new HashMap<String,Object>());
		mv.addObject("durationlist",durationlist);
		return mv;
	}
